...
app.post("/email", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.registerWithEmail({
...
app.post("/emailvaldiate", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.verifyMagicLinkEmail({
...
app.post("/emailauth", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.startAuthMagicLink({
      user_id: user_id,
      username: username,
...
app.post("/emailauthvalidate", async (req, res, next) => {
  ...
  if (authOrSign) {
    try {
      const ress = await authArmor.validateAuthMagicLink({
  ...
  try {
    const ress = await authArmor.signInMagicLink({
      auth_validation_token,
      auth_request_id
    });
...
app.post("/isregistered", async (req, res, next) => {
  ...
  try {
    const resso = await authArmor.checkRegistartion({
      user_id: user_id ?? "",
      user_name: username ?? "",
      timeOutInSeconds
    });
...
app.post("/registerWithAuthenticator", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.registerWithAuthenticator({
      username
    });
    if (ress) {
      res.status(200).send(ress);
...
app.post("/auth", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.startAuth({
      username: username,
      user_id: user_id,
...
app.post("/validateAuth", async (req, res, next) => {
  try {
    const ress = await authArmor.verifyAuthPoll({
...
app.post("/authweb", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.startWebAuth({
      username: username,
...
app.post("/finishAndValidate", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.finishWebAuth({
...
app.post("/webauthinstart", async (req, res, next) => {
  ...
  try {
    const ress = await authArmor.registerWithWebAuth({
      username,
      email_address,
      timeout_in_seconds: 220,
      attachment_type: "any"
...
app.post("/webauthinfinish", async (req, res, next) => {
  try {
    const ress = await authArmor.finishWebRegister({
      ...req.body
    } as any);
